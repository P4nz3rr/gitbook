...
{% code title="# Display network information around you " %}
```bash
...
Provide SSID, WEP hex key, and key index (`wep_tx_keyidx`) in a config file (e.g., `wep.conf`).\
Set `key_mgmt=NONE` for WEP or open networks.
